...
class ToExcel
{
    ...
    function __construct(string $tableType = '')
    {
        $session = Registry::init()->sessions;//new Sessions(); //$this->session; 

        // уходим если нет моделей для вывода
        if (isset($session->nothing))
        {
            debug($session->nothing);
        ...
        if ($searchFor = $session->getKey('searchFor'))
            $searchQuery = (new Search())->search($searchFor);

        // ******* SELECTED MODELS ******* //
        if (!empty($session->getKey('selectionMode')['models']))
            $searchQuery = (new SelectionsModel())->getSelectedModels();

        switch($tableType)
        {
            case 'getXlsx':
                $this->Model = new \models\WorkingCenters($searchQuery);
            break;
            case 'getXlsxFwc':
                $this->Model = new \models\LocationCenters($searchQuery);
            break;
            case 'getXlsxExpired':
                $this->Model = new \models\Overdues($searchQuery);
            break;
            case 'getFileName':
               $this->Model = new \models\Main();
               $this->Model->start = 0;
               $this->Model->perPage = 0;
            break;
        }
        
        /** New Pagination. Starts before main stock query*/
        if ( $tableType !== 'getFileName' ) {
            $this->Model->start = 0;
            $this->Model->perPage = $this->Model->totalModelsCount();
        }
        
        
        $this->Model->getModelsFormStock();
        $this->collectionName = $assist['collectionName'];

        $this->filename = $this->setFileName($searchFor, $assist);
    ...
    protected function setFileName( string|null $searchFor, array $assist ) : string
    {
        if ( $searchFor )
        {
        ...
        $date = date('d.m.Y');
        return $this->Model->translit($collectionName) . '_'. $date;
    }
    ...
    public function getXlsx()
    {
        ...
        $workingCenters = $this->Model->getWorkingCentersSorted();
        foreach ( $workingCenters as &$workingCenter )
        {
            $perf_day = $workingCenter['perf_day']??'';
            $statuses_start = $workingCenter['statuses']['start']??[];
            $statuses_end = $workingCenter['statuses']['end']??[];

            $workingCenter['title'] = $workingCenter['descr'];
            $workingCenter['deadline'] = "Срок 1 день (". $perf_day . " Артикула/день)";
            $workingCenter['statuses']['start'] = $statuses_start['id']??'';
            $workingCenter['statuses']['end'] = $statuses_end['id']??'';
        }
        unset($workingCenter);
        //debug($workingCenters,'workingCenters',1);
        ...
        $ModelRow = $this->Model->row;//$this->getRow();
        $countModelRow = count($ModelRow);
        ...
        {
            $xlsxRow = $this->Model->drawXlsxRow( $ModelRow[$i] );
	...
	public function getXlsxFwc()
	{
		...
		$sheet->setCellValueByColumnAndRow(6, $startRow, 'Дата: '. $this->Model->formatDate(time()));
		$sheet->getRowDimension($startRow)->setRowHeight(20);
        ...
        $ModelRows = $this->Model->row;//$this->getRow();
        $countModelRows = count($ModelRows);
		...
		{
			$thisModel = $this->Model->drawXlsxRow( $ModelRows[$i], true );
            ...
            $sheet->setCellValueByColumnAndRow(8, $startRow, $this->Model->formatDate($lastStatus['date']));
	...
	public function getXlsxExpired()
	{
        ...
        $sheet->setCellValueByColumnAndRow(4, $startRow, 'Дата: '. $this->Model->formatDate(time()));
        $sheet->getRowDimension($startRow)->setRowHeight(20);
        ...
        $workingCentersExpired = $this->Model->getWorkingCentersExpired(true);
