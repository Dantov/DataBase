...
class ModelView extends General 
{
    ...
    public function dataQuery()
    {
        ...
        //$this->registerTable(['stock', 'images', 'rhino_files', 'stl_files', 'ai_files',
        $this->registerTable(['stock', 'images', 'rhino_files', 'stl_files', 'ai_files',
            'service_data','vc_links', 'gems', 'repairs', 'statuses']);

        //debug($this->getRegisteredTables(),'gt',1);

        $stock      = $this->STOCK??null;
        $images     = $this->IMAGES??null;
        $rhinoFiles = $this->RHINO_FILES??null;
        $stlFiles   = $this->STL_FILES??null;
        $aiFiles    = $this->AI_FILES??null;
        $gems       = $this->GEMS??null;
        $vcLinks    = $this->VC_LINKS??null;
        $repairs    = $this->REPAIRS??null;
        $service_data = $this->SERVICE_DATA??null;
        $statuses     = $this->STATUSES??null;

        $this->link(['id'=>$stock],'=',['pos_id'=>$images]);
        $this->link(['id'=>$stock],'=',['pos_id'=>$stlFiles]);
        $this->link(['id'=>$stock],'=',['pos_id'=>$aiFiles]);
        $this->link(['id'=>$stock],'=',['pos_id'=>$rhinoFiles]);
    ...
    {
        //$aq = new ActiveQuery();
        //$stock = $this->registerTable('stock');
        //$vc_links = $this->registerTable(['vc_links']);
        //$images = $this->registerTable(['images']);
        $stock = $this->STOCK;
        $vc_links = $this->VC_LINKS;
        $images = $this->IMAGES;
    ...
    {
        //$aq = new ActiveQuery();
        //$description = $this->registerTable(['users','description']);
        $description = $this->registerTable(['description']);
        $users = $this->USERS??null;
        $this->link(['userID'=>$description],'=',['id'=>$users]);
    ...
    protected function links($id, $vc_3dNum)
    {
        ...
        */
        ...
        $linkQuery = $this->STOCK
            ->select(['id', 'number_3d'])
            ->join($this->IMAGES,['img_name', 'main', 'sketch','pos_id'])
            ->where('id','=',$id)
            ->exe();
    ...
    protected function vc_3dnumExpl($vc_3dnum, $vc_name) : string
    {
        ...
        if ( $this->STOCK->numRows <= 0 ) return $link;
