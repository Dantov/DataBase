...
class Statuses extends AddEdit
{
    ...
    {
        $count = $this->sts->count('c','pos_id')->where('pos_id','=',$this->id)->asOne('c')->exe();
        if ( $count > 1 )
            return false;
    ...
    public function updateStockStatus( array $updData, int $stockID = 0 ) : bool
    {
        ...
        $this->sts->update(table: "stock", row: ['status'=>$statID, 'status_date'=>$statDate], where: ['id','=',$stockID]);

        //if ( $this->baseSql($query) )
        if ( $this->sts->affectedRows )
            return true;
