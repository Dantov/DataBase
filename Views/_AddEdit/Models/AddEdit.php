...
use Views\vendor\core\{ActiveQuery,Registry,HtmlHelper};
...
class AddEdit extends General
{
    ...
    public function getDataLi()
    {
        ...
        foreach ( $dataTables as $name => $data )
        {
            ...
            {
                $a = $html->tag('a')->setAttr(['elemToAdd'=>'',$coll=>'','collId'=>$arrayLi['id']])->setTagText($arrayLi['name'])->create();
                $li = $html->tag('li')->setTagText($a)->create();
                $data_Li[$name] .= $li;
            }
        }
		return $data_Li;
    ...
    public function getGemsLi()
    {
        ...
        foreach ( $dataTables as $name => $data )
        {
            ...
            if ( $name == 'gems_sizes' )
            {
                ...
                {
                    $a = $html->tag('a')->setAttr(['elemToAdd'=>''])->setTagText($value)->create();
                    $gems_Li[$name] .= $html->tag('li')->setTagText($a)->create();
                }
                
                $gems_Li[$name] .= $html->tag('li')->setAttr(['role'=>'separator','class'=>'divider'])->create();
                ...
                foreach ( $data??[] as $arrayLi )
                {
                    $div = $html->tag('div')->setAttr(['class'=>'addElemMore','addElemMore'=>''])->setTagText('+')->create();
                    $a = $html->tag('a')->setAttr(['elemToAdd'=>''])->setTagText($arrayLi['name'])->create();
                    $gems_Li[$name] .= $html->tag('li')->setAttr(['style'=>'position:relative;'])->setTagText($a.$div)->create();
                }
            }
        }
		return $gems_Li;
	...
	public function getNamesVCLi()
    {
        ...
        foreach ( $this->dataTables as $name => $data )
        {
            ...
            foreach ( $data as $arrayLi )
            {
                ...
                $a = $html->tag('a')->setAttr(['elemToAdd'=>'','VCTelem'=>''])->setTagText($arrayLi['name'])->create();
                $vc_namesLI .= $html->tag('li')->setTagText($a)->create();
            }
        }
		return $vc_namesLI;
	}
        ...
        foreach ( $vcLinkRows??[] as $key => $vcLink )
        {
            ...
            foreach ( $result as $res )
            {
                ...
                if ( $vcLink['vc_names'] === $res['model_type'] )
                {
                    ...
                    $a = $html->tag('a')->setAttr(['class'=>'imgPrev','elemToAdd'=>'','imgtoshow'=>$fileImg])
                        ->setTagText($nameVC)->create();

                    $num3DVC_LI[$key] .= $html->tag('li')->setTagText($a)->create();
                }
            }
        }
		return $num3DVC_LI;
    ...
    public function getModelsByType( string $modelType ) : array
    {
        ...
        foreach ( $sorted as $detail )
        {
            ...
            $a = $html->tag('a')
                    ...
                    ->setTagText($nameVC)
                    ->create();
            $resp[] = $html->tag('li')->setTagText($a)->create();
        }
