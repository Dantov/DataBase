...
use Views\vendor\core\{
    ActiveQuery, Model, Sessions, Request, Registry
};
...
class General extends Model
{
    ...
    public function getUsers( bool $full=false ) : array
    {
        ...
        return $this->users = $this->aq->users->select(['id', ...$logPass, 'fio','fullFio','location','access'])->exe();
    }
    ...
    public function getWorkingCentersDB() : array
    {
        ...
        $wc = $this->aq->working_centers->select(['id','name','descr','user_id'])->exe();
        if ( !$this->aq->working_centers->numRows ) 
    ...
    public function getWorkingCentersSorted() : array
    {
        ...
        $centerRows = $this->aq->working_centers->select(['*'])->orderBy('sort_id', 'ASC')->exe();
        if ( !count($centerRows) ) 
     ...
     */
    /*
    public function getStatuses( int $id, string $status_name='', string $status_date='' ) : array
    {
        $c = $this->aq->statuses->count(field: 'status')->where(['pos_id','=',$id])->asOne('status')->exe();
        if ( $c ) return [];
    ...
    */
     ...
     * @throws \Exception
     */
    private static function getServiceArr() : array
    {
    ...
    public function getStatLabArr( string $query, $location = '') : array
    {
		...
		if ( $query == 'image' )
		{
            ...
            $c = 0;
            foreach ( self::getServiceArr() as $imageStatus )
            {
...
//                            debug(User::getAccess(),'User getAccess');
    ...
    public function isStatusPresent(int $statusID = 0, int $stock_id = 0 ) : bool
    {
        ...
        $q = $this->aq->statuses->count('c','pos_id')->where('pos_id','=',$stock_id)->and('status','=',$statusID)->asOne('c')->exe();
