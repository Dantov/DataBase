...
class Main extends General {
    ...
    public function getModelsFormStock( $showall = false )
    {
        ...
        $this->row = $this->findAsArray($selectRow);
	...
	{
        $drawBy = $this->assist['drawBy_'];
        
        $this->wholePos = $result['wholePos'] = count($this->row);
        $this->getWorkingCentersSorted();

        //debug($this->workingCentersSorted,'sorted',1);
        ob_start();
            $source = '';
            switch ($drawBy)
    		{
...
//        debug($locations);
     ...
     * @throws \Exception
     */
    public function getWorkingCentersExpired( bool $xls = false )
	{
