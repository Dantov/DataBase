...
class Search extends General
{
    ...
    {
        if ( !is_string($searchInput) || empty($searchInput) ) return [];
    ...
    public static function byStatusesHistory($connection, $statusID, &$searchedModels, $dates = [])
    {
        ...
        $sqlStr = "SELECT pos_id,status,date FROM statuses WHERE status='$statusID' $andDates AND pos_id IN $in";
        $statuses = $this->findAsArray($sqlStr);
        /*
